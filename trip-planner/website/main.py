...
# 1.Below is the original flask which adopts default static file folder
# app = Flask(__name__)
# 2. Below set the project root directory as the static folder
...
def clusters():
    # 1.Get the file from front-end submission
    file = request.files['file']
    path = os.path.join(r'.\file_uploaded', file.filename)
    file.save(path)
    print('Successful upload:',file.filename)
    rawdatatable = pd.read_csv(path,header=None)

    # 2.Read the table from local storage
    path = r'Berkeley_120.csv'
    rawdatatable = pd.read_csv('Berkeley_120.csv',header=None)
    rawdatatable.columns = ['Longitude','Latitude','NUMBER','STREET','UNIT','CITY','DISTRICT','REGION','POSTCODE','ID','HASH']
    print('Successful download:',file.filename)
...
# @app.route('/Berkeley_120.csv')
# def root():
#     rawdatatable = pd.read_csv('Berkeley_120.csv',header=None)
#     print(rawdatatable)
#     return app.send_static_file('Berkeley_120.csv')
