...
def sentiment_analysis(df):
    ...
    sentiment_df = pd.DataFrame(columns=['business_id', 'name', 'sentiment_polarity', 'sentiment_subjectivity'])
    ...
    for business_id, group in df.groupby(['business_id', 'name']):
        ...
        sentiment_df = sentiment_df.append({'business_id': business_id[0],
                                            'name': business_id[1],
                                            'sentiment_polarity': sentiment_polarity,
                                            'sentiment_subjectivity': sentiment_subjectivity}, ignore_index=True)
...
def lda_model(df, num_topics = 10, remove_stopwords = True):
    ...
    if remove_stopwords:
        ...
        tokenized_reviews = [[word for word in tokens if word.lower() not in english_stopwords] for tokens in tokenized_reviews]
...
def extract_topics(df, lda_model, dictionary, num_words = 10, remove_stopwords = True):
    ...
    def get_topic_words(review):
        ...
        top_words = [word for word, _ in lda_model.show_topic(top_topic[0], topn=num_words)]
...
def extract_keywords(df, num_words=10, split_long=True, remove_stopwords=False):
    ...
    def extract_keywords_from_review(review):
        ...
        if split_long:
            ...
            for chunk in chunks:
                ...
                chunk_tokens = [token for token in word_tokenize(chunk) if token.lower() not in english_stopwords]
    ...
    df_with_keywords['keywords'] = df_with_keywords['text'].apply(extract_keywords_from_review)
...
def content_based_aggregate(df):
    ...
    agg_df = df.groupby(['business_id', 'name', 'city', 'state']).agg({
        ...
        'topic_words': lambda x: [word for word, count in Counter([item for sublist in x for item in sublist]).most_common(20)],
        'keywords': lambda x: [word for word, count in Counter([item for sublist in x for item in sublist]).most_common(20)],
...
def calc_cosine_similarity(df, business_name, my_city, destination_city):
    ...
    if len(target_business) > 1:
        target_business = target_business.sort_values(by=['stars', 'review_count'], ascending=[False, False])
    ...
    cosine_similarities = {}
    for col in ['sentiment_polarity', 'sentiment_subjectivity', 'topic_words_top20', 'keywords_top20', 'reviews']:
        if col == 'reviews':
            # Concatenate all reviews for each business into one string
            target_reviews = " ".join(target_business[col])
            destination_reviews = destination_businesses[col].apply(lambda x: " ".join(x))
            # Compute cosine similarity using TfidfVectorizer to convert text into vectors
            vectorizer = TfidfVectorizer(max_features=1000, min_df=1, use_idf=True).fit_transform([target_reviews] + list(destination_reviews))
            ...
            destination_words = destination_businesses[col].apply(lambda x: " ".join(x)).tolist()
    ...
    similarity_df['business_id'] = destination_businesses['business_id'].reset_index(drop=True)
    ...
    similarity_df = similarity_df[['business_id', 'name', 'city', 'state', 'sentiment_polarity', 'sentiment_subjectivity', 'topic_words_top20', 'keywords_top20', 'reviews']]
    
    # Calculate integer rank for each column
    similarity_df_rank = similarity_df[['sentiment_polarity', 'sentiment_subjectivity', 'topic_words_top20', 'keywords_top20', 'reviews']].rank(ascending=False, method='dense').astype(int)
    similarity_df_rank.columns = ['sentiment_polarity_rank', 'sentiment_subjectivity_rank', 'topic_words_top20_rank', 'keywords_top20_rank', 'reviews_rank']
    similarity_df = pd.concat([similarity_df, similarity_df_rank], axis = 1)
    
    # Sort by ascending rank
    similarity_df = similarity_df.sort_values(by=['sentiment_polarity', 'sentiment_subjectivity', 'topic_words_top20', 'keywords_top20', 'reviews'])
...
def content_based_recommender(df):
    """
        ...
        five features.
    """
    df['avg_rank'] = df[['sentiment_polarity_rank', 'sentiment_subjectivity_rank', 'topic_words_top20_rank', 'keywords_top20_rank', 'reviews_rank']].mean(axis = 1)
